        ...
        for j in range(len(bins)-1):
            ...
            if len(uu)> 150:
                ...
                indice = np.flatnonzero(np.logical_and(insar.uu[uu]>np.percentile(\
                	insar.uu[uu],20.),insar.uu[uu]<np.percentile(insar.uu[uu],80.)))
...
#plt.suptitle('strike: %s, dip: %s'%(strike,fmodel[0].dip))
# fig.colorbar(m1,shrink=0.5, aspect=5)
