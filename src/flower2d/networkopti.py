...
class network(object):
    ...
    def load(self,flt):
        ...
        if 1 == self.dim:     # InSAR case
            ...
            # ── 1. Load raw columns ───────────────────────────────────────────
            if self.los and self.heading:
                logger.info('los=True, heading=True: reading look & head angles per pixel (cols 4 & 5)')
                x, y, los, look, head = np.loadtxt(f, comments='#', unpack=True)
            elif self.los:
                logger.info('los=True, heading=False: reading look angle per pixel (col 4), av_heading for heading')
                if self.phim is None:
                    logger.critical('heading=False but av_heading not defined. Exit!'); sys.exit()
                x, y, los, look = np.loadtxt(f, comments='#', unpack=True)
                head = np.full(len(look), self.phim)
            else:
                logger.info('los=False: using average LOS angles (av_los, av_heading)')
                if (self.phim is None) and (self.profile.proj is None):
                    logger.critical('av_heading not defined and no profile.proj. Exit!'); sys.exit()
                if (self.lookm is None) and (self.profile.proj is None):
                    logger.critical('av_los not defined and no profile.proj. Exit!'); sys.exit()
                x, y, los = np.loadtxt(f, comments='#', unpack=True)
                look = np.full(len(los), self.lookm if self.lookm is not None else 0.)
                head = np.full(len(los), self.phim  if self.phim  is not None else 0.)
            ...
            if self.errorfile is not None:
                ...
                _, _, sigmad = np.loadtxt(self.errorfile, comments='#', usecols=(0,1,2), unpack=True)
                if len(sigmad) != len(los):
                    logger.warning('Error file not the same length as data. Exit!'); sys.exit()
                sigmad = abs(sigmad) * self.wd
            else:
                sigmad = np.ones(len(los)) * self.wd

            # ── 4. Subsample ──────────────────────────────────────────────────
            sl = slice(None, None, self.samp)
            x, y, los, look, head, sigmad = (
                x[sl], y[sl], los[sl], look[sl], head[sl], sigmad[sl]
            )

            # ── 5. Profile projection + spatial filter ────────────────────────
            logger.debug('Compute along/across-profile distances')
            ...
            index = np.nonzero(
                (xp > self.profile.xpmax) | (xp < self.profile.xpmin) |
                (yp > self.profile.ypmax) | (yp < self.profile.ypmin)
            )
            ulos   = np.delete(los, index)
            sigmad = np.delete(sigmad, index)
            look   = np.delete(look, index)
            head   = np.delete(head, index)
            self.x,  self.y  = np.delete(x, index),  np.delete(y, index)
            self.xp, self.yp = np.delete(xp, index), np.delete(yp, index)

            # ── 6. NaN / nodata masking ───────────────────────────────────────
            bad = (ulos == 0.0) | (ulos > 9990.)
            if self.nodata is not None:
                bad |= (ulos == self.nodata)
            ulos[bad]   = np.nan
            sigmad[bad] = np.nan
            self.ulos   = ulos * self.scale
            self.sigmad = sigmad * self.scale

            # ── 7. LOS projection vectors ─────────────────────────────────────
            if self.los or self.heading:
                phi   = np.deg2rad(-90. - head)
                theta = np.deg2rad(90.  - look)
                self.phim   = np.nanmean(phi)
                self.thetam = np.nanmean(theta)
                self.proj  = [np.cos(theta)*np.cos(phi),
            ...
            if self.mask is not None:
                uu = np.flatnonzero((self.yp <= self.mask[0]) | (self.yp >= self.mask[1]))
                self.yp, self.xp   = self.yp[uu], self.xp[uu]
                self.x,  self.y    = self.x[uu],  self.y[uu]
                self.ulos           = self.ulos[uu]
                self.sigmad         = self.sigmad[uu]
                if self.los or self.heading:
                    self.proj = [p[uu] for p in self.proj]
                ...
                if len(uu) > 0:
                    indice = np.flatnonzero(
                        np.logical_and(~np.isnan(self.ulos[uu]),
                            np.logical_and(
                                self.ulos[uu] >= np.nanpercentile(self.ulos[uu], 100-self.perc),
                                self.ulos[uu] <= np.nanpercentile(self.ulos[uu], self.perc)
                            )
                        )
                    )
            ...
            self.Npoint = self.N = len(self.ulos)
            self.N = self.Npoint * self.dim
            self.d = np.atleast_1d(self.ulos)
            if self.base is None:
                logger.info('base=None: setting ramp uncertainties to [10, 0.1]')
                self.base = [10, 0.1]

        elif 2 == self.dim: # GPS case
            logger.debug('Dim = 2, Load East, North GPS')

            name,x,y = np.loadtxt(f,comments = '#',unpack = True,dtype = 'U4,f,f')
            if self.utm_proj is not None:
                  x, y = self.UTM(x, y)
                  x, y = (x - self.ref_x)/1e3, (y - self.ref_y)/1e3
            yp = (x-self.profile.x)*self.profile.n[0]+(y-self.profile.y)*self.profile.n[1]
            ...
            for i in range(self.Npoint):
                ...
                dated,east,north,esigma,nsigma = np.loadtxt(station,comments = '#',usecols = (0,1,2,3,4),unpack = True)
            ...
            if self.errorfile is not None:
                ...
                east,north,self.sigmapar,self.sigmaperp=np.loadtxt(errorfile,comments = '#',usecols = (0,1,2,3),unpack = True)
            ...
            for i in range(self.Npoint):
                station = self.wdir+self.reduction+'/'+ self.name[i]
                dated,east,north,up,esigma,nsigma,upsigma = np.loadtxt(station,comments = '#',usecols = (0,1,2,3,4,5,6),unpack = True)
            ...
            if self.errorfile is not None:
                logger.info('Load error file: {}'.format(self.errorfile))
                errorfile=self.errorfile
                east,north,self.sigmapar,self.sigmaperp,self.sigmav=np.loadtxt(errorfile,comments = '#',usecols = (0,1,2,3,4),unpack = True)
    ...
    def computeFullCovariance(self, distmax=70., every=1., ramp='lin',maskcov=None, xbounds=[None, None], plot=False, outdir=None):
        ...
        if sigm is None or lamb is None:
            ...
            fig = plt.figure(1)
            ax = fig.add_subplot(111)
            histo = ax.hist(z,alpha=0.5,bins=50, density=True, histtype='stepfilled',range=(-3,3))
